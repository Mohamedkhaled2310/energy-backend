...
const importData = async () => {
  try {
    ...
    // Check Inspectors
    const existingInspectors = await Inspector.countDocuments();
    if (existingInspectors === 0) {
        console.log('Inserting Inspectors...');
        await Inspector.insertMany(INSPECTORS);
    } else {
        console.log('Inspectors already exist, skipping.');
    }
    ...
    if (existingStations === 0) {
        ...
        // Create initial visits ONLY if we just created stations
        console.log('Inserting sample visits...');
